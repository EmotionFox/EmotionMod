	...
	public boolean generate(World world, Random rand, BlockPos pos)
	{
		...
		if (pos.getY() < 245 && world.isAirBlock(pos) && this.sapling.canBlockStay(world, pos, state))
		{
			for (int l = -4; l <= 4; l++)
			{
				if (l <= 3 && l >= -3)
				{
					if (world.getBlockState(pos.add(l, height + 1, 0)).getBlock() != EmotionBlock.LOG)
						world.setBlockState(pos.add(l, height + 1, 0), leave);
					if (world.getBlockState(pos.add(0, height + 1, l)).getBlock() != EmotionBlock.LOG)
					...
					for (int y = height - 2; y <= height; y++)
					{
						for (int z = -2; z <= 2; z++)
						{
							if (l == 3 || l == -3)
							{
								...
								if (world.getBlockState(pos1.add(x, 0, z)).getBlock() != EmotionBlock.LOG)
									world.setBlockState(pos1.add(x, 0, z), leave);
								if (world.getBlockState(pos2.add(x, 0, z)).getBlock() != EmotionBlock.LOG)
							...
							if (world.getBlockState(pos.add(x, y, z)).getBlock() != EmotionBlock.LOG)
				...
				for (int x = -1; x <= 1; x++)
				{
					for (int z = -1; z <= 1; z++)
					{
						if (world.getBlockState(pos.add(x, height + 1, z)).getBlock() != EmotionBlock.LOG)
